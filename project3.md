...
layout: default
start: 25 Oct 2023, 00:00 (Europe/Zurich)
index: 6
---

# Project 3: Simply Typed Lambda Calculus

**Hand in:** 08 Nov 2023, 23:59 (Europe/Zurich)
...
types:
    ...
    T ::= "Bool"                      boolean type
        ...
        | "(" T ")"
        | singleLetterIdent           uninterpreted types, e.g., "A"

There are three kinds of concrete types in this language: booleans, natural numbers and function types. The type constructor "→" is
...
It also supports "uninterpreted" types such as `A`, `B`, `C`.
There is no way to actually construct a *value* of such types.
They can be useful to use STLC as a proof system for logic propositions, using the Curry-Howard correspondance.
...
We can define `let` in terms of the existing concepts, and this has the advantage that once the translation (also called "desugaring") is done, no addition
...
is called the external language and the language understood by the evaluator (and type checker) is called the internal language.

_Desugaring of derived forms is very convenient from a scientific point of view (so much that it's become a de facto standard in literature). However, careless desugaring in a practical compiler invites trouble. As our experience with scalac shows, performing desugarings during parsing is not a very good idea, because it: 1) confuses tools, e.g. hyperlinking in IDEs, 2) makes prettyprinting and error messages less helpful, 3) complicates compiler extensibility. In this toy project, you will desugar terms before typechecking, because it's really simple to implement, but we would also like you to understand the associated trade-offs._
...
To add pairs, we can't do the same trick, because there are no existing language features to piggyback onto, so we'll need to extend the existing syntax, evaluation and typing rules:
...
The first form creates a new pair.
The other two are called projections, and extract the first and the second element of a pair, respectively.
We add a new kind of values, pair values, and a new kind of type, for the corresponding pair type. We decide that the pair type constructor
...
## What you have to do
  ...
  1. Implement the `desugar` method, which performs desugaring of the `let` syntax. It should produce a term that is not a `Let` term, nor contain any `Let` sub-term.

  1. Implement the `reduce` method, which performs one step of the evaluation, according to the rules of call-by-value reducer (small step). If none of the rules apply, it should throw `NoRuleApplies` exception containing the corresponding irreducible term.

  1. Implement the `typeof` method, which given a context and a term finds its type. In the case of failure, it should throw a `TypeError` exception containing the ill-typed term. The ill-typed term is the leftmost innermost term that doesn't typecheck, i.e., that doesn't have any typing rules that apply to it.

  1. Implement the `curryHowardProof` method. It should return a "proof" of the logic proposition `((A ∧ B) ⊃ C) ⊃ (A ⊃ (B ⊃ C))` using the Curry-Howard correspondance. First manually translate that proposition into a type, then find a term of that type. The `curryHowardProof` must return a (constant) string that parses as that term.

  1. (Optional further thoughts:) can you come up with additional typing rule(s) for the `let` syntax without desugaring? Consider implementing it, and verify that for any term `t`, `typeof(Nil, t)` behaves the same as `typeof(Nil, desugar(t))`. Can you do something similar for `reduce` as well?
